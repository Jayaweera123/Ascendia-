      ...
      if (projectId) {
        ...
        try {
          const projectResponse = await axios.get(
            `http://localhost:8080/progress/${projectId}/projectprogress`,
            {
              headers: {
          ...
          const projectDetailsResponse = await axios.get(
            `http://localhost:8080/pmanager/${projectId}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
          ...
          const tasksResponse = await axios.get(
            `http://localhost:8080/sengineer/${projectId}/tasks`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
            ...
            const tasksWithProgress = await Promise.all(
              tasksData.map(async (task) => {
                const taskProgressResponse = await axios.get(
                  `http://localhost:8080/progress/${task.taskId}/taskprogress`,
                  {
                    headers: {
                      Authorization: `Bearer ${token}`,
                    },
