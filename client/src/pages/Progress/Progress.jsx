...
const Progress = () => {
  ...
  const [tasks, setTasks] = useState([
    { taskId: 0, taskName: "Closure of the building site", progress: 0 },
  ]);
        ...
        const fetchProjectData = async () => {
          try {
            const projectResponse = await axios.get(
              `http://localhost:8080/progress/${projectId}`,
              {
                headers: {
            ...
            const tasksResponse = await axios.get(
              `http://localhost:8080/progress/${projectId}/taskprogress`,
              {
                headers: {
                  Authorization: `Bearer ${token}`,
                },
  ...
  return (
    <div>
      ...
      <section className="flex">
        <SideNavigationPM open={open} setOpen={setOpen} />
