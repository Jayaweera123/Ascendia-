...
//Material
export const listMaterial = (projectId) =>
  api.get("/store/material/getAllMaterials/" + projectId);

export const createMaterial = (material) =>
  api.post("/skeeperonly/material/createMaterial", material);

export const getMaterial = (materialId) =>
  api.get("/store/material/getMaterialById/" + materialId);

export const editMaterial = (materialId, material) =>
  api.put("/skeeperonly/material/editMaterial/" + materialId, material);

export const searchMaterial = (projectId, query) =>
  api.get(`/store/material/searchMaterial/${projectId}?query=${query}`);

export const inventoryUpdateMaterial = (materialId, material) =>
  api.put(
    "/skeeperonly/material/updateInventory/material/" + materialId,
    material
  );

export const getAllUpdatedMaterials = (projectId) =>
  api.get("/store/material/getAllUpdatedMaterials/" + projectId);

export const searchUpdatedMaterial = (projectId, query) =>
  api.get(`/store/material/searchUpdatedMaterial/${projectId}?query=${query}`);

//Equipment
export const listEquipment = (projectId) =>
  api.get("/store/equipment/getAllEquipment/" + projectId);

export const createEquipment = (equipment) =>
  api.post("/skeeperonly/equipment/createEquipment", equipment);

export const getEquipment = (equipmentId) =>
  api.get("/store/equipment/getEquipmentById/" + equipmentId);

export const editEquipment = (equipmentId, equipment) =>
  api.put("/skeeperonly/equipment/editEquipment/" + equipmentId, equipment);

export const searchEquipment = (projectId, query) =>
  api.get(`/store/equipment/searchEquipment/${projectId}?query=${query}`);

export const inventoryUpdateEquipment = (equipmentId, equipment) =>
  api.put(
    "/skeeperonly/equipment/updateInventory/equipment/" + equipmentId,
    equipment
  );

export const getAllUpdatedEquipment = (projectId) =>
  api.get("/store/equipment/getAllUpdatedEquipments/" + projectId);

export const searchUpdatedEquipment = (projectId, query) =>
  api.get(
    `/store/equipment/searchUpdatedEquipment/${projectId}?query=${query}`
  );
...
class AuthService {
  ...
  static isSkeeperonly() {
    const designation = localStorage.getItem("designation");
    const allowedDesignations = ["Store Keeper"];
    return allowedDesignations.includes(designation);
  }
